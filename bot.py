...
_webhook_runner = None  # Menyimpan instance runner untuk graceful shutdown (Po-5)
...
async def post_init(application: Application):
    ...
    # Inisialisasi Singleton HTTP session (Po-4)
    _http_session = aiohttp.ClientSession()
...
async def ganti_paket_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Tampilkan daftar paket lain untuk dipilih (maks 2x ganti per order awal)."""
    query = update.callback_query
    user_id = query.from_user.id
    ...
    await query.answer()
    try:
        await query.message.edit_caption(text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(keyboard))
    except Exception:
        await query.edit_message_text(text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(keyboard))


async def ganti_paket_konfirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Tampilkan konfirmasi sebelum eksekusi ganti paket."""
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
...
async def ganti_paket_exec(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Batalkan order lama, buat order baru dengan paket pilihan."""
    query = update.callback_query
    user_id = query.from_user.id
    user_name = query.from_user.full_name
    ...
    trans_data = await create_transaction_qris(
        order_id=order_id,
        amount=new_paket["harga"],
        description=f"Hyper Family Buy - {new_paket['nama']}"
    )
    ...
    amount = trans_data.get('amount', new_paket["harga"])
    fee = trans_data.get('fee', 0)
    total_payment = amount + fee

    await save_order(user_id, user_name, new_paket_id, order_id, harga_dibayar=total_payment)

    context.user_data['paket_id'] = new_paket_id
    context.user_data['order_id'] = order_id
    context.user_data['amount'] = new_paket["harga"]
    context.user_data['order_changes'] = changes_used + 1
    ...
    try:
        ...
        timeout_secs = max(60, int((expired_dt2.astimezone(WIB) - now_wib()).total_seconds()))
    except Exception:
        pass

    _start_payment_task(
        context.bot, order_id, new_paket_id, user_id, user_name,
        total_payment, timeout_seconds=timeout_secs
    )
...
async def ganti_paket_batal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Kembali ke tampilan QR tanpa ganti paket."""
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
...
# =================== AUTO-APPROVE & REVOKE JOIN REQUEST (Po-7) ===================
